 ...
 * Copyright 2013 Bastien Pederencino <zeromus.software@gmail.com>
 *
...
QByteArray BrowserMainWindow::saveState(bool withTabs) const
{
    ...
    stream << !m_navigationBar->isVisible(); // DEAD
    stream << !m_bookmarksToolbar->isVisible(); // DEAD
    if (withTabs)
...
{
    return m_viewShowToolBarAction;
}
...
/*-------------------------
ACTION TEXT AND MENU TITLE
-------------------------*/

void BrowserMainWindow::retranslate()
{
    /*To add action text or menu title use this form and change from [name] to the true name:

    [m_buttonMenu]->setTitle(tr("[title]"));
    [m_menuAction]->setText(tr("[text]"));
      */

    m_zbrowserMenu->setTitle(tr("&zBrowser Menu"));
    ...
    m_viewShowToolBarAction->setText(tr("Show Menu Bar"));
    ...
    m_zlinkAction->setText(tr("QuickView"));
...
void BrowserMainWindow::setupToolBar()
{
    ...
    //NEW TAB
    m_navigationBar->addAction(m_tabWidget->newTabAction()); //To add new tab button in the navigation toolbar

    m_navigationBar->addSeparator(); //To add separator in the navigation toolbar

    /*To add button in the navigation toolbar use this form and change from [name] to the true name:

    [m_buttonAction] = new QAction (this);
    [m_buttonAction]->setIcon(QIcon(QLatin1String(":128x128/[icon.png]")));
    [m_buttonAction]->setShortcuts(QKeySequence([Qt::CTRL | Qt::ALT | Qt::Key_X]));
    connect([m_buttonAction], SIGNAL(triggered()),
            this, SLOT([buttonAction]()));
    m_navigationBar->addAction([m_buttonAction]);
      */

    /*To add an icon need to put [icon.png] to "/src/data/128x128/" and add it in "/src/data/data.qrc"*/

    /*To add menu in the navigation toolbar use this form and change from [name] to the true name:

    [m_buttonAction] = new QAction (this);
    [m_buttonAction]->setIcon(QIcon(QLatin1String(":128x128/[icon.png]")));
    [m_buttonMenu] = new QMenu (this);
    [m_buttonAction]->setMenu([m_buttonMenu]);
    m_navigationBar->addAction([m_buttonAction]);
      */
    ...
    m_historyBackAction->setIcon(QIcon(QLatin1String(":128x128/back.png")));
    ...
    m_historyForwardAction->setIcon(QIcon(QLatin1String(":128x128/forward.png")));
    m_navigationBar->addAction(m_historyForwardAction);

    //STOP/RELOAD
    m_stopReloadAction = new QAction(this);
    m_stopReloadAction->setIcon(QIcon(QLatin1String(":128x128/reload.png")));
    m_stopReloadAction->setShortcut(QKeySequence(Qt::Key_F5));
    ...
    //DOWNLOAD BUTTON
    m_downloadManagerAction = new QAction(this);
    m_downloadManagerAction->setShortcut(QKeySequence(Qt::CTRL | Qt::Key_J));
    m_downloadManagerAction->setIcon(QIcon(QLatin1String(":128x128/download.png")));
        connect(m_downloadManagerAction, SIGNAL(triggered()),
            this, SLOT(downloadManager()));

    //EDIT MENU
    m_editMenu = new QMenu (this);
    /*To enable edit menu just need to uncomment after line*/
    //m_downloadManagerAction->setMenu(m_editMenu);

    /*To add button in menu use this form and change from [name] to the true name:

    [m_menuAction] = new QAction([m_buttonMenu]);
    connect([m_menuAction], SIGNAL(triggered()),
                this, SLOT([menuAction]()));
    [m_buttonMenu]->addAction([m_menuAction]);
      */
    ...
    m_editCopyAction->setShortcut(QKeySequence(Qt::CTRL | Qt::Key_C));
    ...
    m_navigationBar->addAction(m_downloadManagerAction);


    //SETTINGS
    m_toolsPreferencesAction = new QAction(this);
    m_toolsPreferencesAction->setIcon(QIcon(QLatin1String(":128x128/setting.png")));
    ...
    connect(m_configMenu, SIGNAL(aboutToShow()),
            this, SLOT(aboutToShowViewMenu()));
    m_toolsPreferencesAction->setMenu(m_configMenu);

        //VIEW
    m_viewShowToolBarAction = new QAction(m_configMenu);
    m_viewShowToolBarAction->setShortcut(QKeySequence(Qt::CTRL | Qt::Key_M));
    connect(m_viewShowToolBarAction, SIGNAL(triggered()), this, SLOT(viewToolBar()));

    m_viewToolbarAction = new QAction(this);
    connect(m_viewToolbarAction, SIGNAL(triggered()), this, SLOT(viewToolbar()));

    m_viewBookmarkBarAction = new QAction(m_configMenu);
    connect(m_viewBookmarkBarAction, SIGNAL(triggered()), this, SLOT(viewBookmarksBar()));
    m_configMenu->addAction(m_viewBookmarkBarAction);

    QAction *viewTabBarAction = m_tabWidget->tabBar()->viewTabBarAction();
    m_configMenu->addAction(viewTabBarAction);
    connect(viewTabBarAction, SIGNAL(toggled(bool)),
            m_autoSaver, SLOT(changeOccurred()));

    m_helpChangeLanguageAction = new QAction(m_configMenu);
    connect(m_helpChangeLanguageAction, SIGNAL(triggered()),
            BrowserApplication::languageManager(), SLOT(chooseNewLanguage()));
    m_configMenu->addAction(m_helpChangeLanguageAction);

    m_toolsSearchManagerAction = new QAction(m_configMenu);
    ...
    m_configMenu->addAction(m_toolsSearchManagerAction);

    m_adBlockDialogAction = new QAction(m_configMenu);
    connect(m_adBlockDialogAction, SIGNAL(triggered()),
            AdBlockManager::instance(), SLOT(showDialog()));
    m_configMenu->addAction(m_adBlockDialogAction);

    m_configMenu->addSeparator();

        //WINDOWS
    m_windowsAction = new QAction(m_configMenu);
    m_windowsMenu = new QMenu(m_configMenu);
    connect(m_configMenu, SIGNAL(aboutToShow()),
            this, SLOT(aboutToShowWindowMenu()));
    aboutToShowWindowMenu();
    m_windowsAction->setMenu(m_windowsMenu);

    m_windowsMenu->addSeparator();

    m_viewStopAction = new QAction(m_windowsMenu);
    QList<QKeySequence> shortcuts;
    shortcuts.append(QKeySequence(Qt::CTRL | Qt::Key_Period));
    shortcuts.append(Qt::Key_Escape);
    m_viewStopAction->setShortcuts(shortcuts);
    m_tabWidget->addWebAction(m_viewStopAction, QWebPage::Stop);
    m_windowsMenu->addAction(m_viewStopAction);

    m_viewReloadAction = new QAction(m_windowsMenu);
    shortcuts.clear();
    ...
    m_windowsMenu->addAction(m_viewReloadAction);

    #if QT_VERSION >= 0x040600 && defined(Q_WS_X11)
    m_viewStopAction->setIcon(QIcon(QLatin1String(":128x128/stop.png")));
    m_viewReloadAction->setIcon(QIcon(QLatin1String(":128x128/reload.png")));
#endif

        //TOOLS
    m_toolsWebSearchAction = new QAction(m_configMenu);
    connect(m_toolsWebSearchAction, SIGNAL(triggered()),
            this, SLOT(webSearch()));
    //m_configMenu->addAction(m_toolsWebSearchAction);

    m_viewSourceAction = new QAction(m_configMenu);
    connect(m_viewSourceAction, SIGNAL(triggered()),
            this, SLOT(viewPageSource()));
    m_configMenu->addAction(m_viewSourceAction);

    m_toolsEnableInspectorAction = new QAction(m_configMenu);
    ...
    m_configMenu->addAction(m_toolsEnableInspectorAction);

    m_toolsUserAgentMenu = new UserAgentMenu(m_configMenu);
    m_configMenu->addMenu(m_toolsUserAgentMenu);

#if QT_VERSION >= 0x040600 || defined(WEBKIT_TRUNK)
    m_viewTextEncodingAction = new QAction(m_configMenu);
    m_configMenu->addAction(m_viewTextEncodingAction);
    m_viewTextEncodingMenu = new QMenu(m_configMenu);
    ...
    //ZOOM SUBMENU
m_viewZoomInAction = new QAction(m_configMenu);
...
connect(m_viewZoomInAction, SIGNAL(triggered()),
        this, SLOT(zoomIn()));
m_configMenu->addAction(m_viewZoomInAction);

m_viewZoomNormalAction = new QAction(m_configMenu);
...
m_configMenu->addAction(m_viewZoomNormalAction);

m_viewZoomOutAction = new QAction(m_configMenu);
...
connect(m_viewZoomOutAction, SIGNAL(triggered()),
        this, SLOT(zoomOut()));
m_configMenu->addAction(m_viewZoomOutAction);

m_viewZoomTextOnlyAction = new QAction(m_configMenu);
...
m_configMenu->addAction(m_viewZoomTextOnlyAction);

    m_configMenu->addSeparator();

        //HELP
    m_helpAboutQtAction = new QAction(m_configMenu);
    connect(m_helpAboutQtAction, SIGNAL(triggered()),
            qApp, SLOT(aboutQt()));
    m_configMenu->addAction(m_helpAboutQtAction);

    m_helpAboutApplicationAction = new QAction(m_configMenu);
    connect(m_helpAboutApplicationAction, SIGNAL(triggered()),
            this, SLOT(aboutApplication()));
    m_configMenu->addAction(m_helpAboutApplicationAction);
    ...
    m_navigationBar->addAction(m_toolsPreferencesAction);


    //HOME
    m_homeAction = new QAction(this);
    m_homeAction->setShortcut(QKeySequence(Qt::CTRL | Qt::ALT | Qt::Key_H));
    connect(m_homeAction, SIGNAL(triggered()), this, SLOT(goHome()));
    m_homeAction->setIcon(QIcon(QLatin1String(":128x128/home.png")));

    //MAIN MENU
    m_zbrowserMenu = new QMenu();
    m_homeAction->setMenu(m_zbrowserMenu);

        //FILE
    m_fileNewWindowAction = new QAction(m_zbrowserMenu);
    ...
    connect(m_fileNewWindowAction, SIGNAL(triggered()),
            this, SLOT(fileNew()));
    m_zbrowserMenu->addAction(m_fileNewWindowAction);

    m_fileOpenFileAction = new QAction(m_zbrowserMenu);
    ...
    connect(m_fileOpenFileAction, SIGNAL(triggered()),
            this, SLOT(fileOpen()));
    m_zbrowserMenu->addAction(m_fileOpenFileAction);

    m_fileOpenLocationAction = new QAction(m_zbrowserMenu);
    ...
    m_zbrowserMenu->addAction(m_fileOpenLocationAction);

    m_fileSaveAsAction = new QAction(m_zbrowserMenu);
    ...
    m_zbrowserMenu->addAction(m_fileSaveAsAction);

    m_zbrowserMenu->addSeparator();

    //BOOKMARK MENU
    m_bookmarksMenu = new BookmarksMenuBarMenu(m_zbrowserMenu);
    ...
    connect(m_bookmarksMenu, SIGNAL(openUrl(const QUrl&, TabWidget::OpenUrlIn, const QString&)),
        m_tabWidget, SLOT(loadUrl(const QUrl&, TabWidget::OpenUrlIn, const QString&)));
    m_zbrowserMenu->addMenu(m_bookmarksMenu);
    ...
    m_bookmarksShowAllAction->setShortcut(QKeySequence(Qt::CTRL | Qt::Key_B));
    ...
    m_fileImportBookmarksAction = new QAction(this);
    connect(m_fileImportBookmarksAction, SIGNAL(triggered()),
        bookmarksManager, SLOT(importBookmarks()));
    m_bookmarksMenu->addAction(m_fileImportBookmarksAction);

    m_fileExportBookmarksAction = new QAction(this);
    connect(m_fileExportBookmarksAction, SIGNAL(triggered()),
        bookmarksManager, SLOT(exportBookmarks()));
    m_bookmarksMenu->addAction(m_fileExportBookmarksAction);
    ...
    bookmarksActions.append(m_fileImportBookmarksAction);
    bookmarksActions.append(m_fileExportBookmarksAction);
    bookmarksActions.append(m_bookmarksAddFolderAction);
    m_bookmarksMenu->setInitialActions(bookmarksActions);

    //HISTORY MENU
    m_historyMenu = new HistoryMenu(m_zbrowserMenu);
    connect(m_historyMenu, SIGNAL(openUrl(const QUrl&, const QString&)),
        m_tabWidget, SLOT(loadUrlFromUser(const QUrl&, const QString&)));

    //m_stopReloadAction->setMenu(m_historyMenu);

    QList<QAction*> historyActions;

    m_historyRestoreLastSessionAction = new QAction(this);
    m_historyRestoreLastSessionAction->setShortcut(QKeySequence(Qt::CTRL | Qt::ALT | Qt::Key_R));
...
#endif
    m_zbrowserMenu->addMenu(m_historyMenu);

    m_zbrowserMenu->addSeparator();

    //zLink
    m_zlinkAction = new QAction(m_zbrowserMenu);
    connect(m_zlinkAction, SIGNAL(triggered()), this, SLOT(gozLink()));
    m_zlinkAction->setShortcut(QKeySequence(Qt::CTRL | Qt::ALT | Qt::Key_Z));
    m_zbrowserMenu->addAction(m_zlinkAction);

    m_viewFullScreenAction = new QAction(m_zbrowserMenu);
    ...
    m_zbrowserMenu->addAction(m_viewFullScreenAction);

    m_filePrivateBrowsingAction = new QAction(m_zbrowserMenu);
    ...
    m_filePrivateBrowsingAction->setShortcut(QKeySequence(Qt::CTRL | Qt::ALT | Qt::Key_P));
    m_zbrowserMenu->addAction(m_filePrivateBrowsingAction);

    m_toolsClearPrivateDataAction = new QAction(m_zbrowserMenu);
    connect(m_toolsClearPrivateDataAction, SIGNAL(triggered()),
            this, SLOT(clearPrivateData()));
    m_zbrowserMenu->addAction(m_toolsClearPrivateDataAction);

    m_zbrowserMenu->addSeparator();

    m_editFindAction = new QAction(m_zbrowserMenu);
    ...
    m_zbrowserMenu->addAction(m_editFindAction);
    new QShortcut(QKeySequence(Qt::Key_Slash), this, SLOT(editFind()));

    m_editSelectAllAction = new QAction(m_zbrowserMenu);
    ...
    m_zbrowserMenu->addAction(m_editSelectAllAction);

    m_filePrintPreviewAction= new QAction(m_zbrowserMenu);
    ...
    m_zbrowserMenu->addAction(m_filePrintPreviewAction);

    m_filePrintAction = new QAction(m_zbrowserMenu);
    ...
    connect(m_filePrintAction, SIGNAL(triggered()),
            this, SLOT(filePrint()));
    m_zbrowserMenu->addAction(m_filePrintAction);

    //m_zbrowserMenu->addSeparator();

    //m_zbrowserMenu->addAction(m_tabWidget->closeTabAction());

    m_fileCloseWindow = new QAction(m_zbrowserMenu);
    ...
    //m_zbrowserMenu->addAction(m_fileCloseWindow);

    m_fileQuit = new QAction(m_zbrowserMenu);
    ...
    //m_zbrowserMenu->addAction(m_fileQuit);
...
#else
        setWindowTitle(tr("%1 - BlueLightCat", "Page title and Browser name").arg(title));
#endif
    }
}
...
void BrowserMainWindow::gozLink()
{
    zLink *zlink = BrowserApplication::zlink();
    tabWidget()->currentWebView()->setContent(zlink->render());
}
...
void BrowserMainWindow::loadProgress(int progress)
{
    if (progress < 100 && progress > 0) {
        ...
        m_stopReloadAction->setIcon(QIcon(QLatin1String(":128x128/stop.png")));
...
{
    m_windowsMenu->clear();
    m_windowsMenu->addAction(m_tabWidget->nextTabAction());
    m_windowsMenu->addAction(m_tabWidget->previousTabAction());
    m_windowsMenu->addSeparator();
    QList<BrowserMainWindow*> windows = BrowserApplication::instance()->mainWindows();
    for (int i = 0; i < windows.count(); ++i) {
        BrowserMainWindow *window = windows.at(i);
        QAction *action = m_windowsMenu->addAction(window->windowTitle(), this, SLOT(showWindow()));
        action->setData(i);
        action->setCheckable(true);
        if (window == this)
            action->setChecked(true);
